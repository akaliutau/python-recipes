...
from messenger.core.bridge.redis_dep import RedisError

from messenger.core.service import (
    MessageService,
    WebServer,
...
def fake_strict_redis():
    with patch(
        'core.bridge.redis.StrictRedis', FakeStrictRedis
...
@pytest.fixture
def uuid4():
    with patch('core.bridge.redis.uuid4') as uuid4:
        yield uuid4
...
def message_lifetime():
    with patch(
        'core.bridge.redis.MESSAGE_LIFETIME', 100
    ) as lifetime:
        yield lifetime
